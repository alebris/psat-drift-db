...
if not positions.empty:
    fig = px.pie(positions, names="quality_class", title="Position quality distribution")
    st.plotly_chart(fig, use_container_width=True)

    positions["ts"] = pd.to_datetime(positions["ts"])
    by_month = positions.set_index("ts").resample("MS").size().reset_index(name="count")
    fig2 = px.bar(by_month, x="ts", y="count", title="Positions per month")
...
if not deployments.empty:
    ...
    fig3 = px.bar(counts, x="manufacturer", y="count", title="Deployments per manufacturer")
